class BroadcasterSpotifyClient
  ...
  def currently_playing_song
    ...
    {
      name: song_name(response),
      uri: song_uri(response),
      millisecond_progress: milliseconds_into_song(response),
    }
  end

  private

  def song_uri(song_request_response)
    return if no_song_playing?(song_request_response)
    JSON.parse(song_request_response.body)["item"]["uri"]
  end

  def milliseconds_into_song(song_request_response)
    return if no_song_playing?(song_request_response)
    JSON.parse(song_request_response.body)["progress_ms"]
  end
